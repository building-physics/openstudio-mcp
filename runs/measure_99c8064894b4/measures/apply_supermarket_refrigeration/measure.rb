class ApplySupermarketRefrigeration < OpenStudio::Measure::ModelMeasure
  ...
  def run(model, runner, user_arguments)
    ...
    # Map existing thermal zones; use fallbacks if named zones not present
    zone_map = {}
    ...
    # Group compressors into MT/LT rack buckets
    mt_racks = {}
    lt_racks = {}
    ...
    # Build condensers — verified OS 3.11 setters only
    condenser_map = {}
    all_condensers = []
    ...
    # Assign one condenser per rack (round-robin fallback if names don't match)
    condenser_index = 0
    mt_rack_count = 0
    ...
    # Build display cases
    # OS 3.11: RefrigerationCase.new(model, defrost_sch) — zone via setThermalZone
    case_count = 0
    objects.select { |o| o['type'] == 'OS:Refrigeration:Case' }.each do |obj|
      zone = zone_map[obj['ZoneName'] || 'MainSales'] || zone_map['MainSales'] || zone_map.values.first
      next unless zone
    ...
    # Build walk-ins
    # OS 3.11: zone boundary set via setZoneBoundaryThermalZone(zone),
    # then scalar setZoneBoundary*(value) — no zone arg on the scalar setters
    walkin_count = 0
    objects.select { |o| o['type'] == 'OS:Refrigeration:WalkIn' }.each do |obj|
      zone = zone_map[obj['ZoneName'] || 'ActiveStorage'] || zone_map['ActiveStorage'] || zone_map.values.first
      next unless zone
      ...
      # Zone boundary: first set the zone, then scalars (no zone arg)
      wi.setZoneBoundaryThermalZone(zone)
      wi.setZoneBoundaryTotalInsulatedSurfaceAreaFacingZone(20.0)
      wi.setZoneBoundaryInsulatedSurfaceUValueFacingZone(0.235)
      wi.setZoneBoundaryAreaofStockingDoorsFacingZone(obj['StockingDoorAreaFacingZone'].to_f) if obj['StockingDoorAreaFacingZone']
      wi.setZoneBoundaryHeightofStockingDoorsFacingZone(2.0)
      wi.setZoneBoundaryStockingDoorUValueFacingZone(obj['StockingDoorUValue'].to_f)          if obj['StockingDoorUValue']
      wi.setZoneBoundaryStockingDoorOpeningScheduleFacingZone(model.alwaysOnDiscreteSchedule)
      walkin_count += 1
    end
