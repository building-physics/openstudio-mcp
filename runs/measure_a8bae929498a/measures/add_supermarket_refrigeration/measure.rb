class AddSupermarketRefrigeration < OpenStudio::Measure::ModelMeasure
  ...
  def run(model, runner, user_arguments)
    ...
    # Pass 3: build compressor lists and rack systems
    objects.each do |obj|
      type = obj['type']
      name = obj['name']
      next unless type == 'OS:Refrigeration:CompressorList'
      cl = OpenStudio::Model::ModelObjectList.new(model)
      cl.setName(name)
      (obj['Compressors'] || []).each do |cname|
        comp = created[cname]
        cl.addModelObject(comp) if comp
      end
      created[name] = cl
    end
