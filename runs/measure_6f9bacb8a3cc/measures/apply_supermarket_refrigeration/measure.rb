class ApplySupermarketRefrigeration < OpenStudio::Measure::ModelMeasure
  ...
  def run(model, runner, user_arguments)
    ...
    mt_racks.each do |rack_key, comps|
      cond_name = "#{rack_key}_Condenser"
      cond_name_mt = "MT_#{rack_key}_Condenser"
      cond = condenser_map[cond_name] || condenser_map[cond_name_mt]
      next unless cond
    ...
    lt_racks.each do |rack_key, comps|
      cond_name_lt = "LT_#{rack_key}_Condenser"
      cond = condenser_map[cond_name_lt]
      next unless cond
      rack = OpenStudio::Model::RefrigerationSystem.new(model)
