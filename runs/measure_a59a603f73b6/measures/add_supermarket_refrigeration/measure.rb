  ...
  def run(model, runner, user_arguments)
    ...
    objects.each do |obj|
      ...
      when 'OS:Curve:Bicubic'
        ...
        c.setCoefficient8xPOW2TIMESY(obj['Coefficient8x2y'].to_f) rescue c.setCoefficient8xPow2TIMESY(obj['Coefficient8x2y'].to_f) rescue nil
        c.setCoefficient9xTIMESYPOW2(obj['Coefficient9xy2'].to_f) rescue c.setCoefficient9xTIMESYPow2(obj['Coefficient9xy2'].to_f) rescue nil
    ...
    objects.each do |obj|
      ...
      sys.setName(name)
      (obj['Compressors'] || []).each do |cname|
        comp = created[cname]
        sys.addCompressor(comp) if comp
      end
      cond = created[obj['Condenser'].to_s]
      sys.setRefrigerationCondenser(cond) if cond
      (obj['Cases'] || []).each do |cname|
        rc = created[cname]
        sys.addCase(rc) if rc
      end
      (obj['WalkIns'] || []).each do |wname|
        wi = created[wname]
        sys.addWalkin(wi) if wi
      end
      created[name] = sys
