class ApplySupermarketRefrigeration < OpenStudio::Measure::ModelMeasure
  ...
  def run(model, runner, user_arguments)
    ...
    # Group compressors into racks by name pattern
    mt_racks = {}
    lt_racks = {}
    ...
    # Build condensers using only verified OS 3.11 API methods:
    # setRatedSubcoolingTemperatureDifference, setRatedFanPower,
    # setCondenserFanSpeedControlType, setMinimumFanAirFlowRatio
    condenser_map = {}
    objects.select { |o| o['type'] == 'OS:Refrigeration:Condenser:AirCooled' }.each do |obj|
      ...
      cond.setCondenserFanSpeedControlType('VariableSpeed') if obj['FanSpeedControlType'] && obj['FanSpeedControlType'].to_s.downcase.include?('variable')
    ...
    mt_racks.each do |rack_key, comps|
      cond = condenser_map["MT_#{rack_key}_Condenser"] || condenser_map["#{rack_key}_Condenser"]
      unless cond
        # Use first available condenser as fallback
        cond = condenser_map.values.first
      end
      next unless cond
    ...
    lt_racks.each do |rack_key, comps|
      cond = condenser_map["LT_#{rack_key}_Condenser"] || condenser_map.values.last
      next unless cond
      rack = OpenStudio::Model::RefrigerationSystem.new(model)
    ...
    objects.select { |o| o['type'] == 'OS:Refrigeration:WalkIn' }.each do |obj|
      ...
      boundary.setTotalInsulatedSurfaceAreaFacingZone(20.0)
      boundary.setInsulatedSurfaceUValueFacingZone(0.235)
